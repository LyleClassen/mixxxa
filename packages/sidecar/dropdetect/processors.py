# Vendored from https://github.com/FelixMeyer6/drop-detector
# Commit: 42890e89c42c142a07ddc8cf71091c3d9bddf158
# License: GPL-3.0 (see LICENSE in this directory)
#
# Trimmed to inference-only. Changes from upstream:
#   - features_batch runs sequentially (no joblib.Parallel / tqdm — process
#     spawning is fragile inside a frozen PyInstaller exe).
#   - prints go to stderr: stdout carries the sidecar's JSON protocol.
#   - removed get_score_at_time (training-only helper).
...
def generate_candidates(fpath, CONF: Dict) -> List[Dict]:
...
def features_batch(df):
    """Computes features for all candidates, sequentially per file."""
    flat_results = []
    for fpath, group in df.groupby("fpath"):
        flat_results.extend(features_file(fpath, group))
